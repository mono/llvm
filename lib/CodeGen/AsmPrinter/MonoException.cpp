...
MonoException::endFunction(const MachineFunction *MF)
{
  ...
  info.HasLandingPads = !MMI->getLandingPads().empty();
  info.Instructions = MMI->getFrameInstructions();
...
MonoException::endModule()
{
  ...
  MCSymbol *tableEndSym = Asm->OutContext.createTempSymbol ("mono_eh_frame_end");
  ...
  for (std::vector<EHInfo>::iterator
      ...
      info.FDESym = Asm->OutContext.createTempSymbol ("mono_fde");
